...
import { membersApi, MemberOrInvitation, InviteMemberRequest, UpdateMemberRoleRequest, Member } from '@/api/members';
import {
  Table,
  TableBody,
  TableCell,
...
function UpdateRoleDialog({ workspaceId, member, children }: { workspaceId: string, member: Member, children: React.ReactNode }) {
    const [isOpen, setIsOpen] = useState(false);
    const queryClient = useQueryClient();
    ...
    const mutation = useMutation({
        mutationFn: (data: UpdateMemberRoleRequest) => membersApi.updateRole(workspaceId, member.id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['members', workspaceId] });
            setIsOpen(false);
        },
...
function MembersTable({ data, workspaceId }: { data: MemberOrInvitation[], workspaceId: string }) {
  const queryClient = useQueryClient();
  const { user } = useAuth();
  ...
  const columns: ColumnDef<MemberOrInvitation>[] = useMemo(() => [
      {
          accessorFn: (row) => (row.type === 'member' ? row.user.name : row.email),
          id: 'memberName',
          header: 'Member',
          cell: ({ row }) => {
              ...
              return (
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          <div className="font-medium">{item.type === 'member' ? name : ''}</div>
      ...
      },
      {
          ...
          cell: ({ row }) => {
                  ...
                  return (
                      <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => resendMutation.mutate(item.id)}>
                                  Resend Invitation
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => item.invitationLink && copyToClipboard(item.invitationLink)}>
                                  Copy Link
                              </DropdownMenuItem>
                              <DropdownMenuItem className="text-destructive" onClick={() => cancelMutation.mutate(item.id)}>
                                  Cancel Invitation
                              </DropdownMenuItem>
                          </DropdownMenuContent>
                      </DropdownMenu>
              ...
              if (item.type === 'member' && item.userId !== user?.id && canManage(item.role)) {
                  return (
                      <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                              <Button variant="ghost" className="h-8 w-8 p-0">
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            This will permanently remove {item.user.email} from the workspace.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
...
function MembersComponent() {
  ...
  if (isError) {
    return <div className="text-destructive">Failed to load members. Please try again.</div>;
  }
