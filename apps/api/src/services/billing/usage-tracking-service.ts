...
import type { ConversionEvent, BillingPeriod, UsageStats } from './types.js';
...
export class UsageTrackingService {
	...
	async recordConversionComplete(event: ConversionEvent): Promise<void> {
		...
		await db.transaction(async (trx) => {
			...
			if (existing.length > 0) {
				const currentBreakdown = existing[0].conversionTypeBreakdown as Record<string, number> || {};
				const newBreakdown = {
					...currentBreakdown,
	...
	async resetMonthlyUsage(): Promise<void> {
		...
		console.log('🔄 Monthly usage reset completed for', previousMonth.toISOString().substring(0, 7));
	}
}